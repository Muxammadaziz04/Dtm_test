...
const Tests = () => {
    ...
    const sendResults = async (e) => {
        ...
        tests[0].tests.forEach(test => {
            if (+form[test.question_id].value === +test.question_variants.correct) first_subject_correct++
        })
        tests[1].tests.forEach(test => {
            if (+form[test.question_id].value === +test.question_variants.correct) second_subject_correct++
        })
